...
<div class="d-none" id="webinarPrerequisitesModal">
    ...
    <form action="/panel/prerequisites/store" method="post">
        <input type="hidden" name="webinar_id" value="{{  !empty($webinar) ? $webinar->id :''  }}">

        <div class="form-group mt-15">
            <label class="input-label d-block">{{ trans('public.select_prerequisites') }}</label>
            <select id="prerequisitesSelect" name="webinar" class="form-control prerequisites-select" data-webinar-id="{{  !empty($webinar) ? $webinar->id : '' }}" data-placeholder="{{ trans('public.search_prerequisites') }}">
        ...
        <div class="mt-30 d-flex align-items-center justify-content-end">
            <button type="button" id="savePrerequisites" class="btn btn-sm btn-primary">{{ trans('public.save') }}</button>
